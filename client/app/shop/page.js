...
export default function ShopPage() {
  ...
  // 1. Existing filter parameters
  const currentCategory = searchParams.get("category") || "";
  const currentSearch = searchParams.get("search") || "";
  const currentBrand = searchParams.get("brand") || "";
  const currentMaxPrice = searchParams.get("maxPrice") || "";
  
  // 🆕 ADD THIS: Read the sort order from the URL parameter bar
  ...
  // 🧪 UPDATED: The Filter & Sort Engine
  const filteredProducts = useMemo(() => {
    // Phase A: Apply your existing filters first
    ...
    // Phase B: Apply sorting directly on top of the filtered items
    // We create a shallow copy [...result] to avoid mutating state directly
    if (currentSortBy === "priceLow") {
      result.sort((a, b) => a.price - b.price); // Ascending order
    } else if (currentSortBy === "priceHigh") {
      result.sort((a, b) => b.price - a.price); // Descending order
    } else if (currentSortBy === "newest") {
      // Sorts using MongoDB's default timestamp field (newest created items first)
      result.sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));
    }

    return result;
  }, [allProducts, currentCategory, currentSearch, currentBrand, currentMaxPrice, currentSortBy]); // Added currentSortBy dependency

  // Helper utility to push parameter modifications safely to URL bar
  ...
  return (
    <main className="min-h-screen bg-gray-50 pt-32">
      ...
      <div className="max-w-7xl mx-auto px-6 grid grid-cols-1 lg:grid-cols-4 gap-8 pb-20">
        ...
        {/* LEFT SIDEBAR: FILTERS (Remains completely unchanged) */}
        ...
        {/* RIGHT SIDEGRID: DISPLAY GRID WITH NEW SORT OPTIONS */}
        <section className="lg:col-span-3">
          <div className="mb-6 flex flex-col sm:flex-row justify-between sm:items-center gap-4 text-gray-900">
            ...
            {/* 🆕 NEW COMPONENT: Sort Dropdown Menu */}
