...
export default function RegisterPage() {
    ...
    const handleSubmit = async (e) => {
        ...
        // Define the registration promise
        const registerRequest = axios.post("http://localhost:5000/api/users", { name, email, password })
            .then((res) => {
                setUserInfo(res.data);
                return res.data;
            });

        // Use toast.promise for the modern UI feedback
        toast.promise(
            registerRequest,
            {
        ...
        <main className="min-h-screen bg-purple-100 flex items-center justify-center p-6 pt-32">
            <div className="w-full max-w-md bg-white p-10 rounded-[3rem] shadow-xl border border-rose-100">
                <div className="text-center mb-8">
                    ...
                    <button 
                        disabled={loading} 
                        className="w-full bg-green-400 cursor-pointer text-white p-5 rounded-2xl font-bold hover:bg-rose-400 transition-all flex items-center justify-center gap-2 shadow-lg active:scale-95 disabled:opacity-50"
                    >
